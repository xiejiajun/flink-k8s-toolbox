package com.nextbreakpoint.command

import com.nextbreakpoint.common.DefaultWebClientFactory
import com.nextbreakpoint.common.DefaultPostCommand
import com.nextbreakpoint.common.model.ApiParams
import com.nextbreakpoint.handler.model.TaskManagerDescriptor

class PostCommandTaskManagerDetails : DefaultPostCommand<TaskManagerDescriptor>(DefaultWebClientFactory) {
    override fun run(apiParams: ApiParams, body: TaskManagerDescriptor) {
        super.run(apiParams, "/taskmanager/details", body)
    }
}

